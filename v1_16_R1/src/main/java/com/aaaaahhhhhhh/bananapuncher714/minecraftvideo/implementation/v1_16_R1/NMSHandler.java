package com.aaaaahhhhhhh.bananapuncher714.minecraftvideo.implementation.v1_16_R1;
...
public class NMSHandler implements PacketHandler {
	...
	@Override
	public void display( UUID[] viewers, int map, int width, int height, ByteBuffer rgb, int videoWidth, int xOff, int yOff ) {
	...
	public void display( UUID[] viewers, Entity[] entities, int[] data, int width ) {
		...
		for ( int i = 0; i < maxHeight; i++ ) {
			...
			Item< Optional< IChatBaseComponent > > item = new Item< Optional< IChatBaseComponent > >( new DataWatcherObject< Optional< IChatBaseComponent > >( 2, DataWatcherRegistry.f ), Optional.of( component ) );
